...
if st.session_state.token and st.session_state.project_id:
    ...
    if search_query:
        ...
        tasks_res = requests.get(
            f"{API_URL}/projects/{st.session_state.project_id}/tasks/", headers=headers
        )
            ...
            for task in tasks:
                ...
                with col1:
                    is_checked = st.checkbox(
                        " ", value=task["is_done"], key=f"task_{task['id']}"
                    )
                ...
                with col2:
                    with st.expander(
                        task["title"] if not task["is_done"] else f"~~{task['title']}~~"
                    ):
                        new_title = st.text_input(
                            "Edit title", value=task["title"], key=f"title_{task['id']}"
                        )
    ...
    if prompt := st.chat_input(
        "What should I do (e.g., 'Plan a release cycle for the new app')"
    ):
    ...
    st.info(
        "Please log in and set a Project ID in the sidebar to start chatting with the AI."
    )
